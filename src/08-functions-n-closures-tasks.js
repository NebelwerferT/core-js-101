...
function retry(func, attempts) {
  return () => {
    try {
      return func();
    } catch (e) { return retry(func, attempts - 1)(); }
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const serialArgs = args.map((arg) => JSON.stringify(arg));
    logFunc(`${func.name}(${serialArgs}) starts`);
    const res = func.call(null, ...args);
    logFunc(`${func.name}(${serialArgs}) ends`);
    return res;
  };
}
